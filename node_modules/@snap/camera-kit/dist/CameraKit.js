...
import { lensExecutionError, lensAbortError, lensVideoPlaybackMutedError } from "./namedErrors";
            ...
            createSession({ liveRenderTarget, renderWhileTabHidden, } = {}) {
                return __awaiter(this, void 0, void 0, function* () {
                    const exceptionHandler = (error) => {
                        ...
                        else if (error.name === "LensCoreVideoPlaybackMutedError") {
                            logger.error(lensVideoPlaybackMutedError("Video playback was muted due to browser autoplay policies. " +
                                "Please prompt the user to interact with the page and unmute audio.", error));
                        }
                        else if (error.name === "LensCoreVideoPlaybackFailedError") {
                    ...
                    if (config.fonts.length > 0) {
                        if (this.lensCore.setSystemFontFamilies) {
                            this.lensCore.setSystemFontFamilies({
                                fontFamilies: config.fonts.map((font) => typeof font === "string"
                                    ? [{ name: "Embedded", data: new ArrayBuffer(0) }]
                                    : [Object.assign(Object.assign({}, font), { data: font.data.slice(0) })]),
                            });
                        }
                        else {
                            const fonts = config.fonts.filter((font) => typeof font !== "string");
                            if (fonts.length > 0) {
                                this.lensCore.setSystemFonts({
                                    fonts: fonts.map((font) => (Object.assign(Object.assign({}, font), { data: font.data.slice(0) }))),
                                });
                            }
                        }
                    }
