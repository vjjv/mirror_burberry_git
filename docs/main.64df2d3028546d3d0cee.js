...
window.addEventListener(LENS_REMOTE_API_EVENT, (event) => {
  ...
  console.log("[Analytics] lens remote api event:", detail)
})
...
// EXTERNAL MODULE: ./node_modules/@snap/camera-kit/dist/index.js + 189 modules
var dist = __webpack_require__(9317);
...
const Settings = {
  ...
  config: {
    ...
    remoteAPISpecId: "4bb4d397-da1d-4a46-a034-b3c85f280398", // From my lenses API section
...
console.log("[RemoteAPI] module loaded", {
  useRemoteAPI: Settings.config.useRemoteAPI,
  remoteAPISpecId: Settings.config.remoteAPISpecId,
})
...
const lensRemoteAPIHandler = {
  ...
  getRequestHandler(request) {
    console.log("REMOTE API HIT:", request)

    console.log("REMOTE API raw request:", {
    ...
    if (action !== "picker" || !hasValidValue) {
      console.warn("REMOTE API: ignored payload (expected { action: 'picker', value: number })", {
        endpointId: request?.endpointId,
        payload,
      })
      return
    ...
    console.log("REMOTE API event dispatched:", eventDetail)
...
const bootstrapCameraKitWithRemoteAPI = async (apiToken) => {
  console.log("[RemoteAPI] bootstrapCameraKitWithRemoteAPI called", {
    apiTokenFromArg: Boolean(apiToken),
    apiTokenFromSettings: Boolean(Settings.config.apiToken),
    remoteAPISpecId: Settings.config.remoteAPISpecId,
  })
  ...
  return await (0,dist/* bootstrapCameraKit */.FY)(
    ...
    (container) => {
      console.log("[RemoteAPI] registering lensRemoteAPIHandler")
...
// EXTERNAL MODULE: ./src/eventsForAnalytics.js
var eventsForAnalytics = __webpack_require__(7965);
...
console.log("[CameraKit] Page loaded")
...
const lensIDFromURL = getQueryParam('lensID');
if (lensIDFromURL) {
  Settings.config.lensID = lensIDFromURL;
  console.log('[CameraKit] Overriding lensID from URL param:', lensIDFromURL);
}
...
// --- Desktop splash logic with allowDesktop param ---
...
if (isDesktop() && !allowDesktop) {
  ...
  // Only run the main app if not on desktop or if allowDesktop=true
  ;(async function () {
  let audioContexts = []
  let monitorNodes = []
